...
Define the dataset $$X \in \mathbb{R}^{n\times m}$$, where $$n$$ is the number of samples, and $$m$$ is the number of dimensions. Define 

$$Z \in \mathbb{R}^{n\times k}$$ as the PCA transformed dataset, where $$k < m$$ is the new dimension. In practice, $$k << m$$.

We first calculate the mean of each dimension on the dataset: $$\mu_j = \frac{\sum_{i=0}^{N-1}X_{i,j}}{N}$$. Then, subtract off the mean for each dimension: 

$$
\hat{X}_{i,j} = X_{i,j} - \mu_{j}
$$
